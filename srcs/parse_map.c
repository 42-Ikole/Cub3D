...
/*   Updated: 2020/03/07 10:37:10 by ikole         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_map_two(t_vars *vars, char line, int m_index, int y)
{
	if (line == '3')
	{
		if (!vars->sprite[1]->width)
			ft_error(vars, "Second sprite not allowed in mandatory part\n");
...
static void	fill_map(t_vars *vars, char *line, int y)
{
	int i;
	...
	while (line[i])
	{
		...
		{
			init_sprite_pos(vars, i, 0);
			vars->map->map[y][i] = '2';
		}
		else
			fill_map_two(vars, line[i], i, y);
		i++;
	}
	vars->map->map[y][i] = '\0';
}
